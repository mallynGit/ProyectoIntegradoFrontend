      ...
      <q-card>
        ...
        <q-card-section>
          <q-input
            ...
            type="textarea"
            autogrow
            input-style="min-height: 100px; max-height: 150px;"
            filled
            readonly
          />
        </q-card-section>
        ...
        <q-tr class="table-body" :props="props">
          ...
          <q-td class="td"
            ><q-btn
              class="ver"
              label="Ver"
              @click="showMotivo(props.row.contenido)"
          /></q-td>
          <q-td class="td">{{ props.row.tipo }}</q-td>
          <q-td class="td">{{
            new Date(props.row.timestamp).toLocaleString("es-ES", {
          ...
          <q-td class="td"
            ><q-btn
              class="ver"
              label="Ver"
              @click="goToItem(props.row, props.row.tipo)"
          /></q-td>
          <q-td class="td text-center"
            ><q-checkbox
              v-model="props.row.resuelto"
              @click="updateResuelto(props.row._id)"
              color="secondary"
          /></q-td>
        </q-tr>
      </template>
...
async function updateResuelto(index) {
  console.log(
    reportes.value.find((x) => x._id === index),
    index,
    reportes.value
  );
...
async function goToItem(item, tipo) {
  switch (tipo) {
    case "Mascota":
      useRouter().push({ path: "/pets/" + item.reportedId });
      break;
    case "Comentario":
      console.log(item, "vale");
      useRouter().push({
        path: `/${item.item.parent.type}/${item.item.parent.petid}`,
      });
      break;
    case "Post":
      console.log(item);
      useRouter().push({
        path: `/pets/${item.item.pet}/posts/${item.item.post}`,
      });
    default:
  }
}
